...
const read_env_array =
  require("../../helpers/setting_reader.helper").readEnvArray;
...
module.exports.registerPost = async (req, res) => {
  ...
  const rs = await user_service.createNewUser(
    req.body,
    refresh_token,
    is_admin
  );
  if (!rs) {
    return res
      .status(400)
      .json({ errs: [http_message.status400_conflict_email.message] });
  }
...
module.exports.loginPost = async (req, res) => {
  ...
  if (!rs) {
    return res
      .status(400)
      .json({ errs: [http_message.status400_login_fail.message] });
  }

  if (rs.refresh_token === "block") {
    return res.status(400).json({ errs: ["Tài khoản đã bị khóa"] });
  }
...
module.exports.forgotPassword = async (req, res) => {
  ...
  if (!rs) {
    return res
      .status(400)
      .json({ errs: [http_message.status400_not_exist_email.message] });
  }
