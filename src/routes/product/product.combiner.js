...
const sub_category_service = require("../../models/SubCategory/subCategory.service");
...
module.exports.addFieldForProduct = async (product) => {
  ...
  //thêm tên danh mục con
  const sub_category = await sub_category_service.getDetailsBySubCategoryId(
    product.sub_category_id
  );
  product.dataValues.sub_category = sub_category;
  ...
  product.dataValues.images =
    await product_image_service.getImageListByProductId(product.product_id);
...
module.exports.getAllProductDetailsByIdArray = async (product_id_array) => {
  for (const item of product_id_array) {
    item.dataValues.product = await this.getAllProductDetailsById(
      item.product_id
    );
  }
