...
module.exports.getNestedCategory = async (req, res) => {
  ...
  for (const item of all_category) {
    const list = await subCategory_service.getAllByCategoryId(
      item.category_id,
      []
    );
...
module.exports.updateCategoryName = async (req, res) => {
  ...
  const rs = await category_service.getDetailByCategoryId(category_id);
...
module.exports.deleteCategory = async (req, res) => {
  ...
  if (list.rows.length > 0) {
    return res
      .status(400)
      .json({ errs: [http_message.status400_exist_sub_category.message] });
  }
